// PROTOTYPE — tres variantes de la Bandeja en `/#/bandeja?variant=A|B|C`.
// Código descartable: el ganador se reescribe en BandejaLogistica.tsx.
...
export function ProductThumb({ item, className = 'size-10' }: { item: LogisticsItem; className?: string }) {
  ...
  return (
    <span className={cn('relative grid shrink-0 place-items-center overflow-hidden rounded-md bg-muted', className)}>
...
export function QuantityTag({ item, size = 'sm' }: { item: LogisticsItem; size?: 'sm' | 'lg' }) {
  const many = item.quantity > 1;
  return (
    <span className={cn(
      'inline-flex shrink-0 items-center justify-center rounded-md font-semibold tabular-nums',
      size === 'lg' ? 'min-w-10 px-2 py-1 text-sm' : 'min-w-7 px-1.5 py-0.5 text-[11px]',
