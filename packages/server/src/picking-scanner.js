...
function normalizedItems(inventory) {
  ...
  return (inventory?.orderItems || []).map((raw, index) => {
    ...
    return {
      ...
      variation: objectValue(normalized?.variation || raw?.Variation),
      color: text(normalized?.color),
      size: text(normalized?.size),
      variantLabel: text(normalized?.variantLabel),
      variantSource: text(normalized?.variantSource),
