...
test('el dominio canónico de LIMBO queda autorizado para CORS y Better Auth', () => {
  assert.deepEqual(CANONICAL_WEB_ORIGINS, ['https://limbo.zentolabs.com']);
  assert.deepEqual(CANONICAL_AUTH_ORIGIN_PATTERNS, ['https://*.zentolabs.com']);
  ...
  assert.equal(isZentolabsOrigin('http://limbo.zentolabs.com'), false);
...
test('WEB_ORIGINS y las URLs públicas de Railway se normalizan a orígenes', () => {
  const env = {
    WEB_ORIGINS: 'https://limbo.zentolabs.com, https://zentofact-web-production.up.railway.app/',
    RAILWAY_PUBLIC_DOMAIN: 'zentofact-api-production.up.railway.app',
    RAILWAY_SERVICE_ZENTOFACT_WEB_URL: 'https://zentofact-web-production.up.railway.app',
