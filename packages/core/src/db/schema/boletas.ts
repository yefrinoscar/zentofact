...
export const boletas = pgTable('boletas', {
  ...
  tipoDocumento: text('tipo_documento').default('03'),
  ...
  uniqueSerieCorr: uniqueIndex('idx_company_serie_corr').on(table.companyId, table.serie, table.correlativo),
  companyBranchIdx: index('idx_company_branch').on(table.companyId, table.branchId),
  fechaIdx: index('idx_fecha_emision').on(table.fechaEmision),
  estadoIdx: index('idx_estado_sunat').on(table.estadoSunat),
  summaryIdx: index('idx_daily_summary').on(table.dailySummaryId),
}));
