...
- **Open the screen.** Navigate to `http://127.0.0.1:3011/#/bandeja`. The header `h1` reads `Bandeja`. Compact layout: a priority bar (group `Prioridad de entrega`) with segments `Vencidos`, `Vencen hoy`, `Vencen mañana`, `Próximos` and `Sincronizar` on its right; a tablist named `Flujo de pedidos` with `Pendientes`, `Listos para enviar`, `Enviados`; a table panel whose header holds channel pills `Todos`, `Falabella`, `Ripley`, `Manual`, a small search `Buscar pedidos`, `Imprimir etiquetas`, and `Marcar todos`. Table columns: `Pedido`, `Productos`, `Entrega`, `Tienda`, `Acción`. There is no store selector.
- **Read pending.** Leave `Pendientes` selected. Run `.cursor/skills/verify-zentofact/scripts/control-zentofact api GET /logistics-inbox?stage=pending .cursor/skills/verify-zentofact/artifacts/<run>/bandeja.json`. HTTP 200. The `Pendientes` badge matches `counts.pending`; the priority segments match `counts.urgency`. Preview rows include Falabella `PV-10001` (overdue, `Marcar listo`), manual `QNC-10010` (tomorrow, `Imprimir`), and Ripley `RP-10020` (later, `Imprimir`, one product line tagged `x3` because the seed inserts three unit lines).
- **Filter by priority.** Click a priority segment: it highlights (`aria-pressed=true`) and the table narrows to that urgency (`GET /logistics-inbox?stage=pending&urgency=tomorrow` returns the same rows). Click again to clear.
