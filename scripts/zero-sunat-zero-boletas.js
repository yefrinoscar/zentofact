...
const REASON = 'Ajuste manual: SUNAT CSV mayo 2026 muestra estas boletas con Total CP 0 y Falabella API no encuentra las ordenes en abril/mayo/junio.';
  ...
  return {
    ...
    auditAdjustments: [
      ...audit,
      {
        ...
        script: 'scripts/zero-sunat-zero-boletas.js',
        reason: REASON,
        previous: {
...
async function main() {
  ...
  try {
    ...
    const beforeResult = await client.query(
      `select b.id, b.company_id, c.nombre as company_name, b.numero_completo, b.order_number,
       ...
       where c.ruc = '20607809136'
         and b.order_number = any($1)
       order by b.numero_completo
       for update of b`,
      [ORDER_NUMBERS],
    );

    if (beforeResult.rows.length !== ORDER_NUMBERS.length) {
      throw new Error(`Se esperaban ${ORDER_NUMBERS.length} boletas, se encontraron ${beforeResult.rows.length}`);
    }

    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const beforePath = path.join(OUT_DIR, `ajuste_monto_0_before_${timestamp}.json`);
    ...
    const afterPath = path.join(OUT_DIR, `ajuste_monto_0_after_${timestamp}.json`);
    ...
    const csvRows = beforeResult.rows.map((before) => {
      ...
      return {
        ...
        igvAntes: round2(before.mto_igv),
        igvDespues: round2(after?.mto_igv),
        motivo: REASON,
      };
    });
    const csvPath = path.join(OUT_DIR, `ajuste_monto_0_resumen_${timestamp}.csv`);
