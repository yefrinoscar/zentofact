...
const COMPANY_ID = 2;
const BRANCH_ID = 2;
const CSV_FILE = process.env.DOLPHIN_JUNE_CSV
...
const OUT_DIR = path.resolve('reports/dolphin-junio-2026');
...
async function main(){
  ...
  const lines=['# Registro EB01 (contingencia) DOLPHIN junio 2026','',`Generado: ${new Date().toISOString()}`,`CSV: \`${CSV_FILE}\``,'',
  ...
  fs.writeFileSync(path.join(OUT_DIR,'registro_eb01_dolphin_junio.md'),`${lines.join('\n')}\n`);
