...
async function insertCreditNote(client, row) {
  ...
  // RUNAPUMA: the 45 EB01 notes affect contingency boletas that are not in the system.
  // Per user decision, register them unlinked (affected_boleta_id null) so the system
  // matches SUNAT in amount; only the 5 B001 notes link to a real boleta and anul it.
  ...
  const result = await client.query(
    `insert into credit_notes (
      ...
      '0','0',$17,'0','0',
      $18,$19,$20,$21,$22,
      $23::jsonb,$24::jsonb,'ACEPTADO',$25,'sistema:importacion-sunat-runapuma-notas-mayo-2026',
      $26,$26
    ) returning id`,
    [
...
async function main() {
  ...
  writeCsv('registro_notas_credito_runapuma_mayo.csv', results, columns);
  ...
  const lines = [
    '# Registro notas de credito RUNAPUMA mayo 2026',
    '',
    ...
    '',
    'Regla aplicada: solo se registran las notas cuya boleta afectada existe en el sistema.',
    'Las notas serie EB01 afectan boletas de contingencia que aun no estan en el sistema y quedan pendientes.',
    '',
    'Detalle: `registro_notas_credito_runapuma_mayo.csv`',
  ];
  fs.writeFileSync(path.join(OUT_DIR, 'registro_notas_credito_runapuma_mayo.md'), `${lines.join('\n')}\n`);
