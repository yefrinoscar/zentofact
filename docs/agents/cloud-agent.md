...
Proof: `GET /products?search=AG301&limit=5` returns `mainSku=AG301` with `stockOnHand` 12, `referencePrice` 189.9 and `commissionAmount` 20. Listings may show 170.5; Nueva venta must still start from 189.9 and show `Ganas S/ 169.90`. Open `#/productos` as superadmin or admin. Seller publications live in the product drawer, not on the master row.
